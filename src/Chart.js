...
class Chart extends Component {
  static propTypes = {
    elevationData: PropTypes.arrayOf(PropTypes.shape({
  ...
  renderLabels() {
    ...
    let minElevation = Math.min(...this.props.elevationData.map(point => point.elevation));
    let maxElevation = Math.max(...this.props.elevationData.map(point => point.elevation));
    return [
  ...
  render() {
    return <View style={styles.container} onLayout={this.handleLayout}>
      {
        this.state.layout && this.props.elevationData.length > 0 ?
        <Surface width={this.state.layout.width} height={this.state.layout.height}>
          <Shape
            d={this.getPath()}
            stroke="white"
            strokeWidth={1}
