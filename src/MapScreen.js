...
class MapScreen extends Component {
  ...
  loadElevation() {
    ...
    let url = line.length > 1 ?
      'https://maps.googleapis.com/maps/api/elevation/json?' +
      `key=${GOOGLE_API_KEY}&samples=50&` +
      `path=${line.map(point => `${point.latitude},${point.longitude}`).join('|')}` :
      'https://maps.googleapis.com/maps/api/elevation/json?' +
      `key=${GOOGLE_API_KEY}&locations=${line[0].latitude},${line[0].longitude}`;
    fetch(url)
      .then(res => res.json())
      .then(({ results }) => {
        this.setState({
      ...
      Alert.alert('Reset', 'Do you wish to reset your current track?', [
        {
          ...
          text: 'Reset',
          onPress: () => { this.setState({ line: [], elevationData: null }); },
          style: 'destructive',
        },
      ]);
    }
  ...
  handleToggleMapTypePress = () => {
    this.setState({ mapTypeIndex: (this.state.mapTypeIndex + 1) % MAP_TYPES.length });
  }
  ...
  render() {
    return <View style={styles.scene}>
      ...
      />
      <Navbar>
        <Text style={styles.navbarText}>{distanceOfPolyline(this.state.line).toFixed(2)} km</Text>
        <View style={{ flex: 1 }} />
        <IconButton
          iconName="trash-a"
        ...
        <IconButton
          iconName="map"
          onPress={this.handleToggleMapTypePress}
        />
      </Navbar>
